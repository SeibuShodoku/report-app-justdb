...
> **状態（2026-06-19）**：本番は **Cloud Run＋IAP** を採用（Vercel Hobby は非商用不可で却下）。
> 以下 **§2 Vercelデプロイは superseded**。本番デプロイ・資源の所在は **`../deployment.md`** を参照。
> 本書で有効なのは **§1 ローカル開発** のみ。
...
- Drive 読み取りは外部SA不可（Workspaceがフォルダ継承を外部に波及させない）。社内ユーザーOAuthで読む（仕様 §7 / `spec/photo-report/slack-photo-report-impl-plan.md` §1a）。
