...
VM 常駐で動かす。queued ジョブを拾い、画像プロキシ経由で写真を取り、**VM の Claude Code をヘッドレス起動**して
`report.json` を書かせ、検証して Supabase `photo_reports` に保存する。
...
- **Google 資格情報は持たない**（写真は必ず report-app の画像プロキシ経由）。
...
REPORT_APP_BASE=https://<vercel>      # ローカル検証時は http://localhost:3000
DRIVE_PROXY_SERVER_SECRET=...          # report-app の .env.local と同値
# 任意
CLAUDE_BIN=claude
